...
- `scripts/`
- `docs/`
- legacy root `.html` files until each page is migrated
...
- `robots.txt`
- `llms.txt`
- public `.html` files in `dist/` generated from `_src/pages/`, legacy root
  pages, or collection data

Do not hand-edit generated files. If a generated file is wrong, change the
owning source file or `scripts/build-site.js`, then run `npm run build`.
...
Current commit policy: commit authored source and legacy root page source only
when no `_src/pages/` file owns that route. Ignore generated deploy output,
generated asset derivatives, remote asset caches, and generated indexes/bundles
that can be recreated from source. `robots.txt` may stay tracked because it is
tiny deploy metadata, but it is still generated by the build.

`npm run check:source` fails if a route exists in both the repository root and
`_src/pages/`, or if a root HTML file is not classified in
`data/source-ownership.json`. That keeps the migration simple: one page, one
owner. `npm run check:structure` ratchets legacy CSS, local screenshot
