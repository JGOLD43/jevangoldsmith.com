...
### Topic flashcards (1.5.4 Update 4)

Open **Home → Flashcards** or **Learning → Flashcards**. Create topic decks, tag/edit cards, archive/restore them, or permanently delete archived cards and their review history. **Make flashcard** beside a book highlight carries its passage and book reference into the private editor; write and check the question and answer before saving. Recall, explanation, comparison and application prompts are supported, with optional separately scheduled reverse cards.

Spaced sessions prioritise reviewed cards that are due, interleave topics, then introduce new material (up to 24 cards). Only one direction of a card appears per session, avoiding immediate answer leakage. Reveal follows the recall attempt. “Again” schedules a ten-minute revisit; successful recalls gain spacing using the existing local heuristic scheduler. This is not Math Academy’s proprietary algorithm or a measurement of mastery. Shuffle draws from the full eligible pool (up to 100 cards) and makes no scheduling/history changes. Daily review counts follow the phone’s local date.

The optional upskilling deck contains original practice prompts and success criteria from JGOLD’s existing learning curriculum, inspired by Justin Skycak’s [Advice on Upskilling](https://www.justinmath.com/advice-on-upskilling/) and [Math Academy’s active practice, spacing and interleaving principles](https://mathacademy.com/pedagogy). Importing again does not overwrite edited cards. Cards, book excerpts and review records stay in the encrypted device database; no AI gateway or cloud upload is used. These additions need no native rebuild once Android 1.5.4 is installed.
