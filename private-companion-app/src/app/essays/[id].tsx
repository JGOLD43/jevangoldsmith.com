    ...
    <SafeAreaView edges={['top']} style={styles.safe}>
      <KeyboardAvoidingView behavior={Platform.OS === 'ios' ? 'padding' : undefined} style={styles.flex}>
        ...
        <ScrollView keyboardShouldPersistTaps="handled" contentContainerStyle={styles.editorContent}>
          ...
          <View style={styles.writingMeta}><Text style={styles.writingMetaText}>{bodyText(body).length.toLocaleString()} characters</Text><Text style={styles.writingMetaText}>{Math.max(1, Math.ceil(bodyText(body).trim().split(/\s+/).filter(Boolean).length / 200))} min read</Text></View>
