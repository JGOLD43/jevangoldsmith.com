...
type SmartView = 'all' | 'overdue' | 'week' | 'no-follow-up' | 'recent' | 'needs-details';
...
function ContactComposer({ visible, onDismiss }: { visible: boolean; onDismiss: () => void }) {
  ...
  const save = async () => {
    if (!name.trim()) return;
    setSaving(true);
    const input: NewRelationshipContact = { name, company, role, email, phone, website, location, tags: tags.split(','), notes, cadenceDays: Number(cadence) || 30, nextFollowUpAt: new Date().toISOString() };
    const contact = await createContact(input);
    setSaving(false);
    if (contact) {
      setName(''); setCompany(''); setRole(''); setEmail(''); setPhone(''); setWebsite(''); setLocation(''); setTags(''); setNotes(''); setCadence('30');
      onDismiss();
...
export default function ContactsScreen() {
  ...
  const importContacts = async () => {
    ...
    try {
      ...
      const response = await DeviceContacts.getContactsAsync({ fields: [DeviceContacts.Fields.Name, DeviceContacts.Fields.Emails, DeviceContacts.Fields.PhoneNumbers, DeviceContacts.Fields.Company, DeviceContacts.Fields.JobTitle, DeviceContacts.Fields.Addresses] });
      const existingNames = new Set(contacts.map((contact) => contact.name.trim().toLowerCase()));
      const existingEmails = new Set(contacts.map((contact) => contact.email.trim().toLowerCase()).filter(Boolean));
      const existingPhones = new Set(contacts.map((contact) => contact.phone.replace(/\D/g, '')).filter(Boolean));
      let imported = 0;
      for (const person of response.data) {
        const name = person.name?.trim();
        const email = person.emails?.[0]?.email?.trim() ?? '';
        const phone = person.phoneNumbers?.[0]?.number?.trim() ?? '';
        const phoneKey = phone.replace(/\D/g, '');
        const hasStableDetail = Boolean(email || phoneKey);
        const isDuplicate = (email && existingEmails.has(email.toLowerCase()))
          || (phoneKey && existingPhones.has(phoneKey))
          || (!hasStableDetail && existingNames.has(name?.toLowerCase() ?? ''));
        if (!name || isDuplicate) continue;
        const address = person.addresses?.[0];
        await createContact({ name, company: person.company ?? '', role: person.jobTitle ?? '', email, phone, website: '', location: [address?.city, address?.country].filter(Boolean).join(', '), tags: ['Imported'], notes: '', cadenceDays: 30, nextFollowUpAt: null });
        existingNames.add(name.toLowerCase());
        if (email) existingEmails.add(email.toLowerCase());
        if (phoneKey) existingPhones.add(phoneKey);
        imported += 1;
      }
      Alert.alert('Contacts imported', imported ? `${imported} ${imported === 1 ? 'person is' : 'people are'} now in JGOLD.` : 'Everyone from your phone is already in JGOLD.');
    } catch (cause) {
  ...
  return <>
    <SafeAreaView edges={['top']} style={styles.safe}><ScrollView keyboardShouldPersistTaps="handled" contentContainerStyle={styles.content}>
      <View style={styles.header}><View><Text style={styles.eyebrow}>PERSONAL CRM</Text><Text style={styles.title}>People</Text><Text style={styles.intro}>{due ? `${due} relationship${due === 1 ? '' : 's'} need a little attention.` : 'A quieter way to stay close to the people who matter.'}</Text></View><Pressable accessibilityLabel="Add a person" accessibilityRole="button" onPress={() => setComposerOpen(true)} style={styles.addButton}><SymbolView name={{ ios: 'plus', android: 'add' }} size={27} tintColor={colors.onAction} /></Pressable></View>
      <View style={styles.sections}><Chip label={`Agenda${due ? ` · ${due}` : ''}`} selected={section === 'agenda'} onPress={() => { setSection('agenda'); setSmartView('all'); setActiveTag('All'); }} /><Chip label="Contacts" selected={section === 'contacts'} onPress={() => { setSection('contacts'); setSmartView('all'); }} /><Chip label="Groups" selected={section === 'groups'} onPress={() => setSection('groups')} /><Chip label="Views" selected={section === 'views'} onPress={() => setSection('views')} /></View>
      ...
      {section === 'contacts' ? <Pressable accessibilityRole="button" onPress={importContacts} disabled={importing} style={({ pressed }) => [styles.importButton, pressed && styles.pressed, importing && styles.disabled]}><SymbolView name={{ ios: 'person.crop.circle.badge.plus', android: 'person_add' }} size={17} tintColor={colors.action} /><Text style={styles.importLabel}>{importing ? 'Importing contacts…' : 'Import from phone'}</Text></Pressable> : null}
