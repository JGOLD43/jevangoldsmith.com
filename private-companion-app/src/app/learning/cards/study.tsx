...
import { CARD_RATINGS, reviewIntervalLabel, scheduleCardReview, type CardRating } from '@/learning/card-scheduler';
...
export default function FlashcardStudy() {
  ...
  const [retry, setRetry] = useState(false), [missedCards, setMissedCards] = useState<ReviewCard[]>([]);
  const ungraded = mode === 'cram' || retry;
  ...
  async function grade(rating: CardRating) {
    if (!card || !revealed || lock.current) return; lock.current = true; setSaving(true);
    try { await reviewLearningCard(card, rating, elapsed.current + Date.now() - started.current); if (rating === 'again') setMissedCards(old => old.some(c => c.id === card.id) ? old : [...old, card]); if (rating !== 'again') setRemembered(value => value + 1); else setMissed(value => value + 1); advance(); }
  ...
  return <SafeAreaView style={styles.safe}><ScrollView keyboardShouldPersistTaps="handled" contentContainerStyle={styles.content}>
    <View style={styles.row}>{button('Close', () => router.back())}<Text style={styles.muted}>{ungraded ? (retry ? 'MISSED ANSWERS · PRACTICE' : 'SHUFFLE · UNGRADED') : 'SPACED REVIEW'}{queue.length ? ` · ${Math.min(index + 1, queue.length)}/${queue.length}` : ''}</Text></View>
    {loading ? <ActivityIndicator color={colors.accent} /> : error ? <Text accessibilityRole="alert" style={styles.text}>{error}</Text> : !card ? <>
      <Text style={styles.title}>{queue.length ? 'Session complete' : 'Nothing to review here'}</Text><Text style={styles.text}>{ungraded ? 'Browse again whenever you want. Your spaced review schedule hasn’t changed.' : queue.length ? `${remembered} recalled · ${missed} to revisit. Your next reviews are saved. Difficult cards return sooner; easy cards are spaced further apart.` : 'Choose another topic, add a card, or return when your next review is due.'}</Text>{!ungraded && missedCards.length > 0 ? button('Practise missed answers now', () => { setQueue(missedCards); setIndex(0); setRetry(true); setRevealed(false); setResponse(''); }) : null}{button('Back to flashcards', () => router.back(), true)}
    </> : <>
      ...
      {!revealed ? <><Text style={styles.muted}>Answer from memory before turning the card. For a problem, work it out first.</Text><TextInput accessibilityLabel="Your answer from memory" multiline value={response} onChangeText={setResponse} placeholder="Think it, say it, or jot it here…" placeholderTextColor={colors.textSecondary} style={styles.input} />{button('Reveal answer', () => setRevealed(true), true)}</> : <>
        {response && card.tags.includes('cloze') ? <Text style={styles.muted}>{response.trim().toLocaleLowerCase() === card.answer.trim().toLocaleLowerCase() ? 'Exact phrase matched. Check that you understand it, too.' : 'Compare your answer below. Different wording may still express the same idea.'}</Text> : null}
        ...
        {ungraded ? button('Next card', advance, true) : <><Text style={styles.muted}>Again: missed it · Hard: correct with difficulty · Good: correct · Easy: effortless. Judge your answer before looking.</Text><View style={{ flexDirection: 'row', flexWrap: 'wrap', gap: 10 }}>{CARD_RATINGS.map(rating => <View key={rating} style={{ width: '47%' }}>{button(`${rating[0].toUpperCase()}${rating.slice(1)} · ${reviewIntervalLabel(scheduleCardReview(card.state, rating).intervalDays)}`, () => { void grade(rating); }, rating === 'good')}</View>)}</View></>}
