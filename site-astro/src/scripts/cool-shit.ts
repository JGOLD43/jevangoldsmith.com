...
function renderFeed(items: AnyObj[], cats: AnyObj[]) {
    ...
    els.feed.innerHTML = items.map((item) => {
        ...
        const linkHtml = item.url
            ? `<a class="feed-item-link" href="${sanitizeUrl(item.url)}" target="_blank" rel="noopener">Visit ${escapeHtml(item.source)} →</a>`
            : '';
        return `
...
function renderTagRail(items: AnyObj[], cats: AnyObj[]) {
    ...
    els.tagRail.addEventListener('click', (e) => {
        ...
        if (!pill) return;
        setFilter(pill.dataset.cat || 'all');
    });
}
...
onDomReady(init, 'cool-shit init');
