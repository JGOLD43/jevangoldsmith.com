// Centralized inline-SVG icon library used by the build-time chrome
// renderer (collection sidebars, nav, footer). Keeping every icon's
// SVG markup byte-stable here so swapping icons doesn't drift between
// pages and so the per-page CSS purge sees consistent class strings.
...
const ICON_SVGS: Record<string, string> = {
  ...
  // Filled-style brand glyphs use fill="currentColor" rather than stroke.
  // Renderers (sprite + getIcon) preserve the original outer attrs.
  ...
  // Mail outline (envelope). 24x24, stroke-based. Drives Newsletter,
  // Email Me, and Contact Page links.
  ...
  // Filled-envelope variant — used by the Email Me link in the contact
  // dropdown; visually distinct from the outline mail glyph.
  envelope: '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M4 4h16c1.1 0 2 .9 2 2v12c0 1.1-.9 2-2 2H4c-1.1 0-2-.9-2-2V6c0-1.1.9-2 2-2z"/><polyline points="22,6 12,13 2,6"/></svg>',
  ...
  letterboxd: '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor"><path d="M8.224 14.352a4.447 4.447 0 0 1-3.775-2.092C3.162 10.611 2.75 8.776 3.014 7.03a7.478 7.478 0 0 1 2.118-4.217A7.214 7.214 0 0 1 9.347 1c1.628-.07 3.215.404 4.553 1.357 1.338.953 2.346 2.318 2.888 3.914a7.523 7.523 0 0 1-1.318 7.116 7.232 7.232 0 0 1-5.996 2.965h-1.25zm7.552 0a4.447 4.447 0 0 0 3.775-2.092c1.287-1.649 1.699-3.484 1.435-5.23a7.478 7.478 0 0 0-2.118-4.217A7.214 7.214 0 0 0 14.653 1c-1.628-.07-3.215.404-4.553 1.357-1.338.953-2.346 2.318-2.888 3.914a7.523 7.523 0 0 0 1.318 7.116 7.232 7.232 0 0 0 5.996 2.965h1.25z"/></svg>',
  // Sidebar / search / close glyphs were inlined per-page until phase 12 —
  // moved here so collection-shell.ts can consume the sprite via getIcon().
  sidebarCollapse: '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><line x1="4" y1="6" x2="4" y2="18"/><polyline points="14 8 10 12 14 16"/></svg>',
  ...
  // Filled check + map-pin used by lesson-logger and north-star.
  ...
  // List/grid view toggles, used by books/adventures view-mode buttons.
  listView: '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><line x1="8" y1="6" x2="21" y2="6"/><line x1="8" y1="12" x2="21" y2="12"/><line x1="8" y1="18" x2="21" y2="18"/><line x1="3" y1="6" x2="3.01" y2="6"/><line x1="3" y1="12" x2="3.01" y2="12"/><line x1="3" y1="18" x2="3.01" y2="18"/></svg>',
  gridView: '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><rect x="3" y="3" width="7" height="7"/><rect x="14" y="3" width="7" height="7"/><rect x="14" y="14" width="7" height="7"/><rect x="3" y="14" width="7" height="7"/></svg>',
  // Adventure-detail back arrow + chevron for movie stats toggle.
  arrowLeft: '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M19 12H5M12 19l-7-7 7-7"/></svg>',
...
// Pull viewBox out of the inline SVG so the <use> wrapper renders correctly.
// Most icons use viewBox="0 0 24 24".
...
// Common stroke/fill/width attrs live on the .ico-* classes in
// legacy-style.css so we don't ship 7 attributes on every icon
// instance — saves ~70 bytes per occurrence × hundreds of occurrences
