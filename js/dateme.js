...
const {
    ...
    stage2Questions,
    toneContent
} = window.JGDateMeContent;

const dateMeState = window.JGDateMeState.create();
const dateMeFlow = window.JGDateMeFlow.create({
    personalityMap,
    stage1Questions,
    stage2Questions,
    state: dateMeState
...
function currentState() {
    return dateMeState.get();
}

function renderLanding() {
    dateMeState.reset();
    window.JGDateMeView.render(funnelContainer, `
...
function renderQuestion() {
    ...
    `, () => {
        if (questionData.type === 'text') {
            window.JGDateMeView.attachQuestionHandlers({
...
function renderContactForm() {
    ...
    `, () => window.JGDateMeView.attachContactHandlers());
}
